  ...
  return (
    <>
      <div className='container'>
        ...
        <button type="button" className="btn btn-secondary mx-2 my-2" onClick={handleLoClick}>LowerCase</button>
        <button type="button" className="btn btn-secondary mx-2 my-2" onClick={handleCopy}>Copy</button>
        <button type="button" className="btn btn-secondary mx-2 my-2" onClick={handleExtraSpaces}>Remove Extra Space</button>
        <button type="button" className="btn btn-secondary mx-2 my-2" onClick={handleClearText}>Clear Text</button>
      </div>
      <div className={`container my-2 text-${props.textMode}`}>
        <h1>Your text sumary</h1>
        <p>{text.split(" ").filter((element)=>{return element.length!==0}).length} words and {text.length} character</p>
        <p>{0.008 * text.split(" ").filter((element)=>{return element.length!==0}).length} Human read in Minutes</p>
        <h2>Preview</h2>
